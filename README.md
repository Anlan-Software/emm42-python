...
- [Session & Connection Options](#session--connection-options)
- [API Reference](#api-reference)
- [Checksum Modes](#checksum-modes)
- [Multi-axis Sync](#multi-axis-sync)
- [Run Tests](#run-tests)
...
所有方法均位于 `MotorDevice`，可通过 `open()` / `close()` 或 `with` 语句管理连接。
...
- `decode_drive_config_response()` — 字段排列根据写命令格式推断，**请在硬件上验证**后再使用 `write_drive_config()`。
